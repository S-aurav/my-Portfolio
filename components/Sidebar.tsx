...
export default function Sidebar() {
  ...
  useEffect(() => {
    ...
    const activeOrder = profile?.sectionOrder
      ? profile.sectionOrder.split(",").map(s => s.trim())
      : defaultOrder;
    const sections = ["hero", ...activeOrder];
  ...
  return (
    ...
    <aside className={`sidebar ${isOpen ? "open" : ""}`} style={{ paddingBottom: '56px' }}>
      ...
      <p className="sidebar-desc">{role}<br />{location}</p>
      ...
      <div className="sidebar-social" style={{ display: 'flex', justifyContent: 'center', gap: '14px', width: '100%' }}>
        ...
        {/* LinkedIn */}
        <a href={linkedin} target="_blank" rel="noopener noreferrer" aria-label="LinkedIn" title="LinkedIn">
          <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
            <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
          </svg>
        </a>
        {/* LeetCode */}
        <a href={leetcode} target="_blank" rel="noopener noreferrer" aria-label="LeetCode" title="LeetCode">
          <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
      ...
      {/* Background river.gif at the bottom left of sidebar */}
      <div
        className="sidebar-bg-gif"
        style={{ position: 'absolute', bottom: 0, left: 0, width: '100%', height: '180px', pointerEvents: 'none', zIndex: 1, display: 'flex', justifyContent: 'flex-start', alignItems: 'flex-end', overflow: 'hidden' }}
      >
        <img
          src="/river.gif"
          alt=""
